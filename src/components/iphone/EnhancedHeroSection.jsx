  ...
  return (
    <motion.button
      ...
      style={{ 
        ...
        transformOrigin: "center" // Aseguramos que la transformación se centre
      }}
    >
...
const EnhancedHeroSection = ({ products = [] }) => {
  ...
  // Gestión táctil optimizada
  const handleSwipe = useCallback((e) => {
    if (iPhoneModels.length < 2 || !e.changedTouches) return;
    const delta = e.changedTouches[0].clientX - e.touches[0].clientX;
    delta > 50 ? prevSlide() : nextSlide();
  }, [iPhoneModels.length]);

  // Handlers memoizados
  const nextSlide = useCallback(() => {
    setActiveIndex(i => (i + 1) % iPhoneModels.length);
  }, [iPhoneModels.length]);

  const prevSlide = useCallback(() => {
    setActiveIndex(i => (i - 1 + iPhoneModels.length) % iPhoneModels.length);
  }, [iPhoneModels.length]);
      ...
      <div className="container mx-auto px-4">
        <div className={`flex flex-col md:flex-row ${isMobile ? 'gap-8' : 'gap-16'}`}>
          ...
          >
            <div 
              id="iphone-carousel"
              className="relative h-80 sm:h-96 md:h-[450px] touch-pan-y flex justify-center items-center"
